...
function showMatch() {
  ...
  if (currentRound >= matches.length) {
    area.innerHTML = `
      <div style="text-align: center; padding: 40px;">
        <h3>🏆 Liga Selesai! 🏆</h3>
        <button onclick="resetLeague()" style="margin-top: 20px; width: auto; padding: 10px 30px;">
          🔄 Buat Liga Baru
        </button>
      </div>
    `;
  ...
  area.innerHTML = `
    <div style="text-align: center;">
      <h3>🎯 Putaran ${currentRound + 1} - Pertandingan ${currentMatch + 1} dari ${round.length}</h3>
      <h4>⚽ ${teamA.name} vs ${teamB.name} ⚽</h4>
      <div class="match-inputs">
        <input type="number" id="scoreA" placeholder="0" value="0" min="0">
        <span>vs</span>
        <input type="number" id="scoreB" placeholder="0" value="0" min="0">
      </div>
      <button onclick="submitScore()">✅ Submit Skor</button>
    </div>
  `;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!hasData) {
    ...
    console.log("Tidak ada data tersimpan, silahkan generate liga baru");
